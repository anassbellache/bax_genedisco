    ...
    def run(self):
        ...
        with sns.axes_style("darkgrid"):
            ...
            for i, acq_file_name in enumerate(glob.glob(root_dir + "/*")):
                ...
                for seed_file_name in sorted(glob.glob(acq_file_name + "/*")):
                    ...
                    for cycle in range(self.num_cycles):
                        results_file_name = os.path.join(
                            seed_file_name, "cycle_" + str(cycle), "test_score.pickle"
                        )
                        with open(results_file_name, "rb") as f:
                            test_indices = pickle.load(f)
                        if seed_name not in methods[acq_name]:
                            methods[acq_name][seed_name] = []
                        methods[acq_name][seed_name].append(
                            test_indices[self.metric_name]
                        )
                ...
                ax.fill_between(
                    x, mean_y - std_y, mean_y + std_y, alpha=0.1, facecolor=clrs[i]
                )
            ax.legend()
