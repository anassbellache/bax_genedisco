...
class SklearnRandomForestRegressor(AbstractMetaModel, SklearnModel):
    ...
    @classmethod
    def load_folder(
        cls: Type["TarfileSerialisationBaseModel"], save_folder_path: AnyStr
    ) -> AbstractBaseModel:
        file_path = os.path.join(save_folder_path, "model.pickle")
    ...
    def get_samples(self, x: np.ndarray, k: Optional[int] = 1):
        ...
        if k > self.n_estimators:
            raise ValueError(
                "The number of requested samples cannot exceed"
                + " the number of estimators in ensemble models."
            )
        else:
            random_indices = np.random.randint(self.n_estimators, size=k)
            y_samples = [self.model.estimators_[i].predict(x) for i in random_indices]
        ...
        if return_std_and_margin:
            ...
            if y_samples.ndim == 3 and y_samples.ndimsy_samples.shape[-1] > 1:
                raise NotImplementedError(
                    "Output uncertainty is only" + "implemented for 1D output."
                )
            else:
    ...
    def predict(
        ...
        while len(row_names) > 0:
            ...
            data = self.merge_strategy_x.resolve(
                dataset_x.subset(list(current_indices)).get_data()
            )[0]

            if return_std_and_margin:
                y_pred, y_std, y_margin = self.get_model_prediction(
                    data, return_std_and_margin=True
                )
...
class PytorchMLPRegressorWithUncertainty(AbstractMetaModel, EmbeddingRetrievalModel):
    def __init__(self, model: TorchModel, num_target_samples: Optional[int] = None):
    ...
    def get_samples(
        self, data: List[torch.Tensor], k: Optional[int] = 1
    ) -> List[torch.Tensor]:
    ...
    def predict(
        ...
        y_preds = list(
            map(
                lambda y_preds_i: torch.cat(y_preds_i, dim=0).detach().numpy(),
                zip(*y_preds),
            )
        )
        ...
        if return_std_and_margin:
            ...
            y_margins = np.squeeze(np.max(y_preds, axis=1) - np.min(y_preds, axis=1))
    ...
    def save_folder(self, save_folder_path, overwrite=True):
        ...
        model_save_path = os.path.join(
            save_folder_path,
            PytorchMLPRegressorWithUncertainty.get_submodel_file_name(),
        )
    ...
    def get_outputs(self, dataset: AbstractDataSource, batch_size: int = 256):
        ...
        return (
            ...
            torch.cat(labels)[:, 0],
            torch.cat(hidden_representations),
        )
    ...
    def get_gradient_embedding(self, dataset, eps: float = 1.0):
        ...
        return (
            hidden_representation * eps * torch.randn_like(y_pred.unsqueeze(-1))
        ).detach()
